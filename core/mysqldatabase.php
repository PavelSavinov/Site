<?php
        class Mysqldatebase
    {
        ...
        const INSERT_GET_AFFECTED_ROW = 2 ;
    ...
    public function fetchOneFromEachRow($query)
    {
        ...
        foreach ($this->interate($query, MySqlResultSet::DATA_NUMERIC_ARRAY) as $row) 
        {
            $rval[] = $row[0] ;
        }
    return $rval ;
 ...
 public function importSqlFile($filename, $callback=false, $abort_on_error=true)
    ...
    public function insert ($query, $r_type=Mysqldatebase::INSERT_GET_AUTO_INCREMENT_ID)
    {
        $r = $this->query($query) ;

        if ($r_type == Mysqldatebase::INSERT_GET_AFFECTED_ROWS)
    ...
    public function query($query)
    {
        ...
        if (!$r)
        {
            throw new Exception ("Querry Error: " . mysql_error()) ;
        }
        return $r ;
    }
    public function quickQuerry($query)
    {
         $r = @mysql_query($query, $this->link) ;

         if (!$r) return false ;
         if (is_resourse($r)) mysql_free_result($r) ;
    ...
    {
        return $this->updateOrDelede($query) ;
    }
